...
public class CandidateServiceImpl implements CandidateService {
    ...
    public List<CandidateDto> findAll() {
        List<Candidate> candidates = repository.findAllPartyMembers().orElseThrow(() -> new DataNotFoundException("No Candidates present in system!"));
        return candidateMapper.mapEntityListToDtoList(candidates);
    }

    @Override
    public CandidateDto findById(Long id) {
        Candidate candidate = repository.findById(id).orElseThrow(() -> new DataNotFoundException("No Candidate with id: " + id + " present in system!"));
    ...
    public List<CandidateDto> findPartyMembersByParty(String partyName) {
        List<Candidate> partyMembers = repository.findAllPartyMembers().orElseThrow(() -> new DataNotFoundException("No Candidates belonging to party: " + partyName + " present in system!"));
        return candidateMapper.mapEntityListToDtoList(partyMembers);
    }

    @Override
    public List<CandidateListResponse> findCandidatesWithOnlyPartyName() {
        return repository.findCandidatesWithOnlyPartyName().orElseThrow(() -> new DataNotFoundException("No Candidates present in system!"));
    }
